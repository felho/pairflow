...
import type { BubbleMergeError } from "../../../src/v11/application/merge/mergeCommandOrchestration.js";
import type {
...
describe("mergeBubble remote routes", () => {
  ...
  it("imports proven remote commit continuity before rejecting stale local state for started remote merge", async () => {
    ...
    await writeStateSnapshot(bubble.paths.statePath, buildBubbleStateSnapshotVariant(staleState), {
      expectedFingerprint: loaded.fingerprint,
      expectedState: "DONE"
    });
