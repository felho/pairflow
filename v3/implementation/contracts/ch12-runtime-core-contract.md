...
"C25": "the staging + one-channel discipline realized ACROSS the window exactly as ratified: the ch8-C21 `{path,message}` channel with container-precondition suppression on every new container (P2 A1/A2 + P3 R1–R3 + P4 A1 — the P4-deferred Map interception retired at P4); the value-level semantics landing WITH their consuming packets (P1a S8/S12/E2/X3, P1b G3/L3, P2 A-lanes, P3 R2 migration refusal co-landing with the guard retirement); the source-form walk + `--mode` verb surface at P4 (F1–F5/V2); the in-handler CLI bridge built at P1b (W2) and retired at P4 (V4); the trace-harness contract + fixture re-bases riding each stage (P1b W3/W4) and the requirement/spec types landing admission-normalized (P3 T1); the context-free deferred-hold journey (P1b J1/J2 — l0dJourney.test.ts + cli/journey.test.ts) and the trace staging honored (both golden traces at P3 — l0dTrace/l0eTrace.test.ts); the RP6 hostile forms (merge-key/alias) driven (P4 dimension 4, validate.test.ts/load.test.ts); the shipped local-pair-v0@1.yaml gained NO new keys under the ch8-P2 equality pin",
