...
export async function buildApplicationDefaultsBoundaryCheckReport({
  ...
  const details = [
    ...filtered.violations.slice(0, 100).map(
      (violation) =>
        violation.severity === "fail"
          ...
          : `${violation.fromRelative}:${String(violation.line)} [warn] application dynamic-imports defaults runtime wiring -> ${violation.toRelative}`
    ),
    ...
    `dynamic_defaults_imports=${String(filtered.violations.filter((violation) => violation.severity === "warn").length)}`,
  ...
  if (problemCount > 0) {
    const violationSummary =
      failCount > 0
        ? `${String(failCount)} application->defaults import(s)`
        : undefined;
