...
We also need to think about this. Maybe it doesn't need too much change from this perspective, but we need to figure out how this system can run in a way that the bubbles could run in a sandbox on a different machine if it is needed. Save and locally can be a sandbox, or it can run on a server. There are some vendors for running sandboxes, or I have a server like a Mac Mini, and I can run the bubbles on that machine. We need to think through what it means from an architecture perspective, especially if the repository is local but the bubble can run in a sandbox. We need to assume that maybe the connection is not working for some time.
