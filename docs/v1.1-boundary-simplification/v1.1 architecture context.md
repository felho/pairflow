...
5. State-et irni kizarolag ket engedelyezett ponton szabad (ket-kapus modell):
   - **Forward mutation (BubbleMutationRunner)**: normal command flow-ban. Kotelezo: STS-validalt `validated_next_state`, transcript-first ordering, fingerprint guard.
   - **Backward reconstruction (TranscriptStateReconciler)**: operator recovery flow-ban. Kotelezo: `reason` + `operation_id` + audit event, transcript-bol derivalt state (nem kezi input).
   - Harmadik state-write kapu NEM letezhet. Minden mas state write tiltott.
   - Lasd: [ADR-001: Dual-Gate State Write Model](./decision-log.md#adr-001-dual-gate-state-write-model).
...
2. `Transcript-first` + dual-gate ADR referenciak formalis publikalasa (jelenleg: [decision-log.md](./decision-log.md), kesobb szetbonthato `docs/adr/*` file-okra).
...
Rollout policy:
1. Ezek target-architektura checkek, nem implicit "azonnali hard fail" szabalyok minden meglvo pathra.
2. Bevezetes fazisosan tortenik: eloszor report-only, majd checkenkenti hard-fail aktivalas migration kapuhoz kotve.

1. Boundary fitness: domain retegben tilos I/O import (`fs`, `child_process`, tmux/git adapterek).
2. Mutation fitness (dual-gate): state write kizarolag `BubbleMutationRunner`-ben (forward mutation) vagy `TranscriptStateReconciler`-ben (backward reconstruction) tortenhet. Harmadik kapu tiltott. Lasd: [ADR-001](./decision-log.md#adr-001-dual-gate-state-write-model).
3. Transition fitness (dual-gate): forward command pathon kotelezo `StateTransitionService.applyStateTransition()` hivasa `BubbleMutationRunner` elott. Reconstruction pathon (reconciler) STS nem alkalmazhato — transcript-bol derivalt state, nem forward transition. Lasd: [ADR-001](./decision-log.md#adr-001-dual-gate-state-write-model).
...
2. Fitness check spec dokumentum (`docs/architecture/architecture-fitness-checks.md`).
