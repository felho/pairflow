...
1. az elso kuszob marad a `watchdog_timeout_minutes`, de ennek uj defaultja ebben a setupban `30` perc,
2. timeout utan csak akkor eszkalalunk, ha a celpane nyers `tmux capture-pane` tartalma legalabb 10 perce valtozatlan, vagy a session/pane mar nem olvashato.
...
4. A standard `RUNNING` watchdog eszkalacio gate-je valtozzon `timeout + pane quiet-window` modellre.
5. Timeout utan a kovetkezo eros jelek azonnali eszkalaciot okozhatnak:
...
2. Uj bubble state, transcript schema vagy public CLI surface.
...
1. Public watchdog result shape maradjon valtozatlan, ahol lehet; ne vezessunk be uj user-facing reason stringet csak emiatt.
...
4. Ha timeout utan nincs elozo pane-activity record, az elso watchdog futas seedelje a recordot, es emiatt ne eszkalaljon meg ugyanabban a futasban pusztan a quiet-window alapjan.
...
| CS5 | `src/v11/shared/watchdog/watchdogCommandRouting.ts` | `resolveWatchdogLifecycleRoute(...) -> Promise<BubbleWatchdogResult>` | existing | standard RUNNING expiry branch | A `RUNNING` expired agban az eszkalacio csak akkor tortenik meg, ha a quiet-window feltetel teljesul vagy a sampler/session eros hibajelet ad. | P1 | required-now | replaces `expired => escalate` with two-step gate |
...
| Watchdog escalation decision | `expired` eleg a standard RUNNING eszkalaciohoz | `expired` + (`quiet_window_reached` vagy `session_missing` vagy `pane_unreadable`) kell | `watchdog_timeout_minutes`, `last_changed_at` | `last_sample_status` | behavior-changing internal, CLI surface kept stable | P1 | required-now |
...
|---|---|---|---|---|---|
| Runtime FS store | read/create/update `.pairflow/runtime/watchdog-health/<bubbleId>.json` | bubble state/trancript schema modositas | Kulon runtime artifact, nem canonical workflow log. | P1 | required-now |
...
| Timeout utan a pane quiet-window elerte a 10 percet | pane-activity record | result | standard watchdog eszkalacio a meglevo HUMAN_QUESTION pathon | existing `escalated` | info | P1 | required-now |
...
| Pane-activity record JSON hibas/olvashatatlan, de uj minta veheto | runtime FS | fallback | record ujraepitese friss mintabol; nincs azonnali quiet-window eszkalacio | existing no-op reason | warn | P2 | required-now |
...
| T3 | timeout expired but pane changed recently | timeout mar lejart, `last_changed_at` < 10 perc | watchdog fut | nincs eszkalacio, state marad `RUNNING` | P1 | required-now | fixes false-positive long task escalation |
| T4 | timeout expired and quiet window reached | timeout mar lejart, `last_changed_at` >= 10 perc | watchdog fut | `HUMAN_QUESTION` + `WAITING_HUMAN` meglevo write pathon keresztul | P1 | required-now | two-threshold escalation |
| T5 | timeout expired and missing session | timeout mar lejart, runtime session nincs | watchdog fut | azonnali standard eszkalacio | P1 | required-now | hard dead-signal handling |
| T6 | timeout expired and unreadable pane | timeout mar lejart, `capture-pane` fail | watchdog fut | azonnali standard eszkalacio | P1 | required-now | pane read failure handling |
