...
source layout under `docs/v3/convergence/model-src/`, rebuilt byte-identically
...
- **Edit sources, never the rendered HTML.** The workflow is: edit
  `model-src/` → `build.py` → `check.sh` must pass before commit. A direct
...
- **Model-side decisions** live in the corpus (binding form in
  `core-model.html` / the convergence docs) with the *why* in a
  `docs/v3/topics/` memo. The memos already ARE the model's decision records:
