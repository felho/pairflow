...
fuller ownership + operator-recovery modeling first — until then configuring it is rejected as
`gate_config_not_supported`, so the MVP stays fail-closed); the `projection_ref` + scoped-query SDK seam (replacing
