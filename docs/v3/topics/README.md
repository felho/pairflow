...
| [`_open-kernel-primitives.md`](_open-kernel-primitives.md) | **COMPLETE — the rebaseline executed as waves 1–5, all ratified (2026-07-06)** — the five kernel primitives (Errand · ChoicePoint · Admission · Warrant · Directive) are named contracts in the corpus at their earned birth points (Admission@L0d · Warrant@L1 · Errand@LC2 · ChoicePoint+Directive@L3) with instance/phase labels corpus-wide; behavior-neutrality machine-checked per wave (registries + per-block rejection multisets; one deliberate ratified delta, F-W4-2). §9 is the per-wave findings log. Remaining named debt: the dedicated F-W1-2 ingress/idempotency hardening touch. | Realized in `core-model.html` (model-src units); `core-model-todo.md` Parts A/B/D/E cross-refs; the F-W1-2 touch's future small-spec. |
