...
| [`_open-kernel-primitives.md`](_open-kernel-primitives.md) | **Decided + ratified — rebaseline unblocked** — the five kernel primitives (Errand · ChoicePoint · Admission · Warrant · Directive) with instance maps; all four review decisions recorded (contracts-in-code; in-place rebaseline at the earned points; the ①②③④→LC1–LC4 + L0f+→L0g rename-pass has LANDED; transports unified at the primitive level; naming via a six-lens anti-anchored brainstorm); five review rounds + the executed L5 paper test (§8, PASS, ratified) in the status changelog. Next: the in-place rebaseline, level by level. | The Phase 3 content refactor of `core-model.html`; `core-model-todo.md` Parts A/B/D/E cross-refs. |
