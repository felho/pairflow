...
| [`_open-kernel-primitives.md`](_open-kernel-primitives.md) | **Open — draft under review** — candidate base primitives (Exchange · Guarded Keyed Selection · Admission ladder · Authority binding · Ask/Intent family) that would dissolve the kernel's additive complexity; instance maps + de-bias tests + the L5 paper test. | The Phase 3 content refactor of `core-model.html`; `core-model-todo.md` Parts A/B/D/E cross-refs. |
