...
|---|---|---|
| [`_open-emit-contract.md`](_open-emit-contract.md) | **Paper test executed (PASS) — awaiting review.** The last Block A model backlog as one slice: todo Parts E (emit contract) + F (policy-vs-verify gate families, evidence currency) + the A1 digest/`op_id_collision` refinement — 11 declarations over existing contracts, zero new machinery; E6 (claim model) and E1's extended fields deliberately out. | The emit-contract section build (`core-model.html`); `core-model-todo.md` Parts E/F closure; v1 parity (`pass.ts`/`converged.ts`). |
