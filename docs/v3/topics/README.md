...
|---|---|---|
| [`_open-emit-contract.md`](_open-emit-contract.md) | **BUILT (2026-07-07) — section EC landed, awaiting the landing review.** The last Block A model backlog as one slice: todo Parts E + F + the A1 digest, as the paper test's 11 declarations with both review rounds folded (help under the generic validator; same-function offer; 4 new rejection names, 81→85; digest scoped to the actor-emit path). Inventories exactly as pre-declared (140/116/85), mirror clean; two ratified behavior deltas (digest + offer); F-EC-1 applied (todo F4 two-concept fix). E6 + E1's extended fields stay as named Absents. | Section EC in `core-model.html`; `core-model-todo.md` Parts A1/E/F REALIZED statuses; v1 parity incl. the contract machine. |
