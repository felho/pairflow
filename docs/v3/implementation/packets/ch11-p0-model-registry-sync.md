...
| S6 | The mutation boundary is the evidence file's CLOSED list — the three mirrors + the count-pinning lock test + this packet file, NOTHING else; NO runtime/compiler behavior rides the bridge. |
...
```json
{
  "packet_rows": {
    "rows": [
      ...
      { "id": "S9", "class": "anchored", "refs": ["prose:ch11-model-sync-delta.md Lane 4 addendum (ratification pending)"] }
    ]
  }
}
```
...
scope (evidence: the mutation boundary is four mechanical-mirror files +
this packet; zero production behavior — S6). Site × shape × phase grid:
N/A — no failure lanes over a phased seam (the packet's only "lanes" are
the three drift locks themselves).

## Pre-approval flags

- **Flag 1 — approve under three red lanes (the ratified bridge).** At
  approve, `rejectionNames.test.ts` (2 tests), `unitMap.test.ts` (1),
  `domainRegistry.test.ts` (1) are red BY DESIGN — authorized by the
  user-ratified §5.5 one-off exception (@ de33d245); the delta-equality
  receipts ride the pre-approval summary. Route: approve-ratified — the
  approve act consumes the already-ratified authorization; revisit: none.
