...
lane texts — the rows this chapter's ratification act REOPENS; C20
single-authority admission; C21 container preconditions; C36 the
workspace-emptiness backstop; the C13/C16/C37 camelCase rename
culture); the ch6-P4a/P4b CLI canonical
...
C1; `runtime_context` requirement grammar VIA the named C18/C19
reopen (ownership move + the bare `required` compat decision;
extended by the panel to C21's two lane texts) →
C2/C3/C4/C5 (+ the prepared reopen texts below); run-override
...
**The prepared ch11 C18/C19/C21 reopen texts** (applied to
`ch11-gate-format-contract.md` by the reopen commits; the ratification
act that ratifies THIS draft resolves them — one human act, per plan
§12.1 item 3, extended to C21 because two of its lane texts hard-code
the retired value domain; pointer form, so the grammar has ONE owner):
...
- C19 (reopened): *The process↔workspace lane: a template declaring
  ANY `external.process` gate whose `runtimeContext` requirement is
  not a provisionable spec map yields the
  `runtime_context_required_for_process_gate` ADMISSION issue — the
  trigger's grammar is owned by `contract:ch12-runtime-core#C5`.
  The model's HANDLE `ready(∅)` runtime backstop (C36) is unchanged.*
- C21 (reopened — the two `runtimeContext` lane texts ONLY, the rest
  ...
  grammar — owned by `contract:ch12-runtime-core#C2–#C4`"; the lane
  "process gate without `runtimeContext: required`" reads "process
  gate without a provisionable `runtimeContext` requirement → code
  `runtime_context_required_for_process_gate` (C19, grammar
  `contract:ch12-runtime-core#C5`)". A packet deriving its lane
  checklist from C21 thereby derives the POST-reopen value domain.*

**Reopen choreography note (for the ratifying act):** the ch11 draft
is `realized` (chapter-closed) — the template's own lifecycle calls a
post-close change a STOP, and plan §12.1 item 3 IS that STOP's
pre-resolved mandate; this is the FIRST realized-chapter reopen
(ch11's own 2026-07-12 reopen ran from `ratified`), so the act
consciously blesses the escape-hatch use, not a lifecycle arrow.
Mechanics keeping every commit lint-green: reopen commit 1 = the
three row edits + the `realized_map` block REMOVED + status →
`reopened` (map presence requires `realized`); re-ratification
commit 2 = the new ratification block (recording commit 1) + the map
RESTORED + status → `realized`. The three restored map entries record
the REOPEN ITSELF — the historical landing kept, plus "reopened at
the ch12 ratification — successor <per-row>" with the PER-ROW
successors (C18 → `#C2–#C4`; C19 → `#C5`; C21 → both) — never a
forward promise of unbuilt ch12 landings. Commit 2's new
ratification block carries its own template-§5 metrics line, and
commit 2 also updates the ch11 Close-metrics reopenings line (1 → 2).
...
ratification's mandated ch11 C18/C19/C21 reopen), the draft passes
...
**Ratification decision points** (the digest's spine — each is the
human's, resolved by the one ratification act unless split off):

1. **The C18/C19/C21 reopen + the bare-`required` retirement** (C2;
   plan §12.1 item 3's mandated act; reality check below — no
   startable file breaks).
2. **The ch8-C14 narrowing at the step position** (C7: `agentConfig`
   becomes map-only where it was any-value — the §8.2 rule 3 MECHANIC
   carried in the realizing row per the consciously-extended ch11-C38
   precedent, ch8 NOT reopened; the asymmetry vs the reopened
   runtimeContext rows is deliberate and leaves ch8-C14's page
   unnarrowed — PERMANENTLY: the GO accepts that ch8-C14's realized
   text will read stale against C7, with C7's packet cross-link duty
   as the mitigation; tree sweep: zero authoring files, paper-only
   break).
3. **The READY terminal-sink model fix** (C15: the panel surfaced
   that the ratified `RUNTIME_CONTEXT_READY` unit carries no
   terminal-sink rung — a post-CANCEL late READY would resurrect a
   cancelled run against `terminal-is-a-sink`; the prepared fix adds
   the `kernel_status ≠ TERMINAL` state expectation to the l0d + l0e
   READY units, mirroring FAIL — registry-neutral, no new rejection
   name (bare-REQUIRE guard). Recommended: fix the model FIRST (the
   `6dd8bd15` precedent), then ratify; C15 is never realized without
   it.)
4. **Silently-contestable stances riding flag-free otherwise** (the
   ...
   recourse is floor visibility + CANCEL), C16 (EMPTY production
...
| C5 | The process↔workspace ADMISSION lane (the C19 successor, trigger re-grammared): a template declaring ANY `external.process` gate whose requirement is `none` (authored or absent) yields the `runtime_context_required_for_process_gate` ADMISSION issue — a process gate needs a PROVISIONABLE workspace, and only a spec map provisions. Driven by the process registration's `requires_runtime_context` flag read by `admit_definition` (unchanged mechanism, template-grain single finding). The HANDLE `ready(∅)` runtime backstop keeps the SAME name as a rejection (ch11-C36, untouched — it now reads the real lifecycle field per C14). |
...
| C15 | The `RuntimeContextProvider` PORT contract (model-verbatim members): `provision(instance_id, request_id, spec)` — async, fire-and-forget from the kernel's view; its completion fires `RUNTIME_CONTEXT_READY(instance_id, request_id, ref)` through the in-process event seam (C13) — and `project_for_actor(ref) → RuntimeContextProjection`. The completion seam is ORDERED-AFTER-COMMIT (the model's "async → later fires" made binding): the composition delivers a READY completion into RECEIVE only AFTER the provisioning START's atomic commit has landed — a provider completing synchronously inside `provision()` (the scripted player may) is HELD by the seam, never lost to the pre-commit correlation window (`runtime_context` still `none` would reject it); if that commit never lands (CAS restart superseded the request), the held READY fails correlation and is inert. The ref and the projection are CANONICAL-JSON-SAFE VALUES BY PORT CONTRACT (finite, acyclic, plain data — C11 stores the ref, C17 carries the projection into the packet); a provider return violating that domain is a kernel/config INTEGRITY throw, fail-closed — never stored or projected lossily. Correlation is the SAFETY, not provider discipline: a duplicate, unsolicited, or already-resolved READY event fails the admission correlation rung (`runtime_context = requested(request_id)` no longer holds) and mutates NOTHING — a bare-REQUIRE guard rejection, no state change; and a READY reaching a TERMINAL instance is rejected on the terminal-sink STATE rung (the l0d/l0e model fix that ratification decision point 3 prepares — mirroring FAIL's `kernel_status ≠ TERMINAL` guard; this row is never realized without it). Provisioning FAILURE has NO port channel at ch12 — DECIDED HERE, the named Absent's honest surface: READY is the port's ONLY completion; a failed provision simply never fires it, the run stays `CREATED` + `requested` — VISIBLE on the floor (C21) and cancellable via CANCEL (the operator recourse); the model prose's failure→`FAIL` routing arrives WITH the provisioning-failure-handling Absent (its wire shape, correlation, and reason domain are that later chapter's rows), and nothing at ch12 fires `FAIL` for a provider — no retry, no health machinery. |
