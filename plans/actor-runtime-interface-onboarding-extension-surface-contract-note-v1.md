...
6. Current-tree source anchors (public surface, read-only):
...
| `prompt concern` | reusable instruction-surface modul, mai source: `build*Guidance` family (peldaul `buildPairflowCommandGuidance`, `buildReviewerSeverityOntologyReminder`, `buildImplementerEvidenceHandoffGuidance`) | proposed companion catalog (`promptConcernCatalog`); naming proposal itt, kod-szintu felepites `O3-T2`-ben |
...
4. A 4 seam azonban a kerneltol fuggetlenul tovabbra is szetszorja az `AgentRole` enum konkret ertekeit:
   ...
   - S4: a 4 startCommand prompt builder file role-specifikus top-level builderekkel (`buildImplementerStartupPrompt`, `buildReviewerStartupPrompt`, `buildMetaReviewerStartupPrompt`, `buildResumeImplementerStartupPrompt`, `buildResumeReviewerStartupPrompt`, `buildResumeMetaReviewerStartupPrompt`); a tenyleges concern modulok (`build*Guidance`) mar reusable formaban leteznek, csak a top-level compose role-szerint elagazo.
...
1. `promptConcernCatalog` (proposed):
   ...
   - kulcs: `PromptConcernId` zart literal union; az ID-k a meglevo `build*` fuggvenyekre lekepezhetoek
   - example mai concern jeloltek (kod-szintu lockolas `O3-T2`-ben):
     - `pairflow_command_guidance` (`buildPairflowCommandGuidance`)
     - `canonical_actor_emit_lookup_guidance` (`buildCanonicalActorEmitLookupGuidance`)
     - `reviewer_severity_ontology_reminder` (`buildReviewerSeverityOntologyReminder`)
     - `reviewer_decision_matrix_reminder` (`buildReviewerDecisionMatrixReminder`)
     - `reviewer_agent_selection_guidance` (`buildReviewerAgentSelectionGuidance`)
     - `reviewer_scout_expansion_workflow_guidance` (`buildReviewerScoutExpansionWorkflowGuidance`)
     - `reviewer_pass_output_contract_guidance` (`buildReviewerPassOutputContractGuidance`)
     - `reviewer_canonical_command_gate_lines` (`buildReviewerCanonicalCommandGateLines`)
     - `reviewer_findings_pass_instruction` (`buildReviewerFindingsPassInstruction`)
     - `meta_review_submit_command_template` (`buildMetaReviewSubmitCommandTemplate`)
     - `meta_review_submit_approve_parity_note` (`buildMetaReviewSubmitApproveParityNote`)
     - `implementer_evidence_handoff_guidance` (`buildImplementerEvidenceHandoffGuidance`)
     - `repository_launch_workspace_line` (`buildRepositoryLaunchWorkspaceLine`)
     - `launch_workspace_command_scope_line` (`buildLaunchWorkspaceCommandScopeLine`)
     - `repo_launch_workspace_task_line` (`buildRepoLaunchWorkspaceTaskLine`)
     - `document_primary_artifact_reviewer_guardrail` (`buildDocumentPrimaryArtifactReviewerGuardrail`)
   - a tenyleges entry-lista lockolasra kerul `O3-T2`-ben; az `O3-T1`-ben ez naming proposal **kod nelkul**.
...
| `startup_prompt_concern_ids` | `["pairflow_command_guidance", "implementer_evidence_handoff_guidance", "canonical_actor_emit_lookup_guidance", "launch_workspace_command_scope_line", "repository_launch_workspace_line"]` (proposed; lockolas `O3-T2`) | `startCommandImplementerPrompts.ts::buildImplementerStartupPrompt` |
| `resume_prompt_concern_ids` | proposed; lockolas `O3-T2` | `startCommandResumeImplementerPrompt.ts::buildResumeImplementerStartupPrompt` |
...
| `startup_prompt_concern_ids` | `["pairflow_command_guidance", "reviewer_severity_ontology_reminder", "reviewer_decision_matrix_reminder", "reviewer_agent_selection_guidance", "reviewer_scout_expansion_workflow_guidance", "reviewer_pass_output_contract_guidance", "canonical_actor_emit_lookup_guidance", "reviewer_findings_pass_instruction", "reviewer_canonical_command_gate_lines", "launch_workspace_command_scope_line"]` (proposed; lockolas `O3-T2`) | `startCommandPrompts.ts::buildReviewerStartupPrompt` |
| `resume_prompt_concern_ids` | proposed; lockolas `O3-T2` | `startCommandResumePrompts.ts::buildResumeReviewerStartupPrompt` |
| `handoff_id_format_id` | `null` (default format) | `executionContext.ts::buildExecutionContextHandoffId` (else branch) |
| `active_agent_constraint_id` | `null` | - |
...
| `startup_prompt_concern_ids` | `["meta_review_submit_command_template", "meta_review_submit_approve_parity_note", "canonical_actor_emit_lookup_guidance", "pairflow_command_guidance", "repository_launch_workspace_line"]` (proposed; lockolas `O3-T2`) | `startCommandPrompts.ts::buildMetaReviewerStartupPrompt` |
| `resume_prompt_concern_ids` | proposed; lockolas `O3-T2` | `startCommandResumePrompts.ts::buildResumeMetaReviewerStartupPrompt` |
...
1. `O3-T2` csak ezen note exact `RoleDescriptor` mezohalmazat es per-role closed mappingjat formalizalhatja kod szinten.
