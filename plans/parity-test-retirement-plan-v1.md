...
task_tracker:
  ...
  - task_id: 2-cli-entrypoint-boundary-guard
    task_path: plans/archive/tasks/2026-04-30-parity-test-retirement-plan-v1/2-cli-entrypoint-boundary-guard.md
    status: archived
...
| `4-meta-review-gate-case-rename` | `plans/tasks/4-meta-review-gate-case-rename.md` | Rename/reclassify meta-review-gate parity-named behavior cases as v11 cases and update corpus expectations. | `3-reconcile-contract-v11-only` | Useful meta-review-gate cases still look like migration parity cases. | approved |
...
2. Task 2:
   ...
   - `pnpm typecheck`,
   - `pnpm lint`.
   - In `docs_only_context`, these implementation checks are not run, not
     satisfied, and not claimed. Use the authoritative
     `docs_only_context` definition and allowed parent-plan alignment field
     scope from
     `plans/archive/tasks/2026-04-30-parity-test-retirement-plan-v1/2-cli-entrypoint-boundary-guard.md#docs-only-context-definition`;
     the canonical PASS summary must include `Context mode: docs_only_context`
     and state `runtime checks were intentionally not executed`.
3. Task 3:
