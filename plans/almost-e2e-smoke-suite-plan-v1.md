...
| `3-actor-loop-smoke` | `plans/tasks/3-actor-loop-smoke.md` | Add one minimal runner-driven fake actor scenario through pass, convergence, and meta-review `approve` recommendation, using canonical `pairflow agent emit --kind ...` feedback. | `1-smoke-runner-contract`, `2-cli-lifecycle-smoke` | Proves actor ingestion, authority refresh, transcript/state progression, and approval-ready routing without a real LLM. | in_progress |
