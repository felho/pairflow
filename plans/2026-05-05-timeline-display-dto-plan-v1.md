...
|---|---|---|---|---|---|
| `1-timeline-rules-fixtures` | `null` | Inventory the current timeline display rules and add/adjust golden fixtures so the existing behavior is testable before migration. Production code changes are out of scope except test-only extraction if needed. | N/A | The current behavior is implicit inside React helpers and can regress during migration. | not_created |
