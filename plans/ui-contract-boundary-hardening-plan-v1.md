...
active_task_id: 3a-action-response-validation
...
| `3a-action-response-validation` | `plans/tasks/3a-action-response-validation.md` | Add runtime validation for selected mutation/action result payloads, starting with delete, commit, and merge, plus same-dispatch siblings only if they share the exact validation adapter shape. | `2b-ui-import-migration` | Action result wire-shape drift can still silently pass through `unknown` dispatch paths. | approved |
