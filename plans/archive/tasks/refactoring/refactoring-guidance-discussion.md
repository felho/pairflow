...
|---|---|---|---|---|
| A1 | Keep this discussion draft as the working design note until the final guidance lands. | `plans/archive/tasks/refactoring/refactoring-guidance-discussion.md` | completed | This archived file preserves the earlier discussion; it is no longer the current source for refactoring policy. |
...
| A3 | Pilot the Module Depth Check manually on one current Boundary/Architecture refactor candidate before changing skills. | removed transient pilot note | completed | The temporary pilot note was removed after the guidance landed. The durable guidance and archived meta-review submit pipeline task now carry the retained context. |
...
| A6 | After durable docs and `CreatePairflowSpec` guidance land, archive this discussion draft and remove the original task path. | `plans/archive/tasks/refactoring/refactoring-guidance-discussion.md` | completed | Full discussion is archived here; the original task path was removed to prevent a redundant source of truth. |
