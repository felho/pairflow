...
3. Read-path rule:
   ...
   - `list`: defaultban local projection + cache-first remote consume, `--refresh` eseten explicit remote refresh,
   - `list` started remote bubble eseten a top-level lifecycle projection (`state`, `round`, `byState`) cache-derived remote statebol vagy explicit refreshbol allhat elo; live remote truth helyett nem maradhat csendben local `state.json`,
...
6. Missing-data rule:
   ...
   - `list` default pathban hianyzo/invalid cache eseten a bubble marad remote-kent jelolt, de a top-level `state` / `round` csak explicit degraded local-control-plane projectionkent jelenhet meg `stateSource=local_control_plane_degraded` + `cacheStatus=missing|invalid` markerrel; ezt live remote truthkent tilos ertelmezni,
...
| `BubbleStatusView` JSON/text projection | `statusCliRunner`, `statusCliRenderers`, CLI/tests | additive | remote execution/location/freshness view mezok hozzaadasa | attach/read-model tovabbi consume `Phase 2F` |
| `BubbleListEntry` / `BubbleListView` projection | `listCliCommand`, core list tests, CLI/tests | additive | remote location/freshness/cache-source mezok hozzaadasa | attach/read-model tovabbi consume `Phase 2F` |
...
|---|---|---|---|---|---|
| started remote live refresh | remote pointer shape + remote target resolution + remote snapshot payload validity | cache overwrite, fresh remote claim, attach/restart wording | fail-closed `status`; per-bubble degraded `list --refresh` | P1 | required-now |
| started remote plain `list` cache miss | remote pointer shape + cache readability/validity | implicit SSH, unlabeled local-state substitution | degraded local-control-plane projection explicit source markerrel | P1 | required-now |
...
| CS4 | `src/v11/shared/list/listCommandApi.ts` | `listBubbles` | `(BubbleListInput -> Promise<BubbleListView>)` | entry build loop / refresh policy | Cache-first remote list projection; started remote bubble top-level lifecycle mezoi cache-derivedek, cache missnel explicit degraded source marker kell; explicit `refresh` path bubble-nkent | P1 | required-now | `tests/core/bubble/listBubbles.test.ts`, `tests/v11/application/list/listCommandApi.test.ts` |
...
| `BubbleListEntry` | local state/runtime/attention projection | additive remote projection metadata + explicit source semantics | existing list fields retained; started remote bubble eseten top-level `state`, `round` cache-derived vagy explicit degraded source markerrel jelolt | `remoteExecution` view: `alias`, `host`, `pointerKind`, `stateSource`, `cacheStatus`, `remoteClonePath?`, `lastCacheCheckAt?` | non-breaking | P1 | required-now |
...
| started remote plain `list` with missing/invalid cache | local cache | fallback | remote bubble kept visible with explicit degraded local-control-plane projection; `stateSource=local_control_plane_degraded`, `cacheStatus=missing|invalid`; no implicit SSH | `LIST_REMOTE_CACHE_UNAVAILABLE` | warn | P1 | required-now |
| started remote `list --refresh` per-bubble SSH failure | SSH remote status | fallback | keep stale cache if present, else degraded unavailable projection for that bubble only | `LIST_REMOTE_REFRESH_UNAVAILABLE` | warn | P1 | required-now |
...
| T5a | list cache miss degraded projection | started remote bubble, missing/invalid `state-cache.json`, started pointer present | `pairflow bubble list` | zero SSH call; explicit degraded local-control-plane projection `stateSource=local_control_plane_degraded` + `cacheStatus=missing|invalid` | P1 | required-now | `tests/core/bubble/listBubbles.test.ts` |
