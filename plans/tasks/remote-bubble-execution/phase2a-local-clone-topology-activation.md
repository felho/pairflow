...
4. A fo implementacios kockazat nem a guard eltavolitasa, hanem a resume ownership/reclaim seam:
   - `src/v11/application/start/startCommandSession.ts` `resolveInitialRuntimeLaunchWorkspaceAuthority(...)` jelenleg a resume initial claim authorityt `launchWorkspacePath = bubblePaths.worktreePath` es `launchWorkspaceKind = bubbleConfig.work_mode` alapjan szintetizalja,
   - `resolveRetryRuntimeLaunchWorkspaceAuthority(...)` ugyan ma megtartja a persisted `workspacePath`-ot, de explicit Phase 2A-s szabaly nelkul visszaesne az initial synthetic authorityra,
...
6. Public contract decision:
   ...
   - activation utan a local `start` pathon a `WORKSPACE_MODE_CLONE_NOT_ACTIVATED` mar nem maradhat retained public outcome; ennek retire-je a Phase 2A local start surface-en az `src/v11/application/start/startCommandApi.ts` es `src/v11/application/start/startCommandLaunchWorkspace.ts` ownershipa, authority-hiany/fallback-tiltas eseten a public fail-closed contract `START_LAUNCH_WORKSPACE_UNAVAILABLE`,
...
| Retired reason-code rule | A local Phase 2A start surface-en `WORKSPACE_MODE_CLONE_NOT_ACTIVATED` nem maradhat success-adjacent vagy authority-failure public outcome. | A retirementet az API/preflight es launch-authority seamen kell lezarni; authority-hiany/fallback-tiltas eseten a public contract `START_LAUNCH_WORKSPACE_UNAVAILABLE`, state precedence eseten `START_STATE_NOT_STARTABLE`. | P1 | required-now |
...
| local start public reason-code surface (`startCommandApi.ts` + `startCommandLaunchWorkspace.ts`) | local `start` command callers, `startBubble.test.ts`, contract harness | breaking | a `WORKSPACE_MODE_CLONE_NOT_ACTIVATED` local-start retirementjet explicit negative proof kotelesseggel kell lezarni; Phase 2A authority-failure lane `START_LAUNCH_WORKSPACE_UNAVAILABLE`, lifecycle-precedence lane `START_STATE_NOT_STARTABLE` | remote start/read-model successor tasks |

Corpus compatibility rule:
1. Phase 2A-ban az elso preferencia a meglevo `start-clone-not-activated*` contract file-ok es runner scenario slotok in-place szemantikai atvezetese.
2. A historic alias filename/scenario token megtarthato, de ilyenkor a leirasnak, expected kimenetnek es runner invariantnek mar activation-success vagy authority-missing fail-closed semantics-et kell bizonyitania; a token nem igazolhat retained pre-claim clone rejectet.
3. Fajlnev/manifest atnevezes csak akkor kotelezo, ha az implementacio tenylegesen uj pathot vezet be; a naming cleanup nem prerequisite a Phase 2A lezarasahoz.
...
| clone-only legacy fallback canonical authority nelkul | forbid | resume/fresh fail-closed teszt, explicit `START_LAUNCH_WORKSPACE_UNAVAILABLE` authority-missing proof | P1 | required-now |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CS1 | `src/v11/application/start/startCommandApi.ts` | `resolveStartBubblePreflightOrThrow(...)`, `startBubble(...)` | `(input) -> Promise<ResolvedStartBubble>`, `(input, dependencies?) -> Promise<StartBubbleResult>` | preflight gate a bubble lookup + state validation utan | a config-only clone reject megszunik; a local start clone bubblesnel is tovabbmegy a normal start mode/authority gate fele, mikozben a `START_STATE_NOT_STARTABLE` precedence retained marad es a local start API surface-en a `WORKSPACE_MODE_CLONE_NOT_ACTIVATED` retire-je explicit | P1 | required-now | T1, T3, T7 |
...
| CS3 | `src/v11/application/start/startCommandSession.ts` | `claimRuntimeSessionOwnership(...)`, `resolveInitialRuntimeLaunchWorkspaceAuthority(...)`, `resolveRetryRuntimeLaunchWorkspaceAuthority(...)`, `inferPersistedLaunchWorkspaceKind(...)` | `(input) -> Promise<RuntimeSessionRecord>`, helperek | ownership claim es stale reclaim seam | `resolveInitialRuntimeLaunchWorkspaceAuthority(...)` csak az input-arg lane-re vonatkozik: freshnel ures authority, worktree resumenel megengedett same-authority preseed, clone resumenel viszont nem generalhat input-alapu clone authorityt; explicit clone authority csak claimed/persisted recordbol (`firstClaim.record`, `existingRecord`) maradhat fenn, es a kind-inference csak persisted `workspacePath` jelenlete mellett elfogadhato | P1 | required-now | T3, T5 |
...
| CS5 | `src/v11/application/start/startCommandApi.ts`, `src/v11/application/start/startCommandLaunchWorkspace.ts` | preflight reject removal + launch-authority fail-closed reason-code surface | existing exports | public start reason-code seam | a local start path nem hasznalhat tovabb `WORKSPACE_MODE_CLONE_NOT_ACTIVATED` reason/message szerzodest ott, ahol Phase 2A mar aktiv; authority-hiany vagy tiltott fallback eseten a public fail-closed `START_LAUNCH_WORKSPACE_UNAVAILABLE`, mikozben a `START_STATE_NOT_STARTABLE` precedence retained marad. `startCommandRuntime.ts` csak akkor scope, ha maradt olyan helper/uzenet, amelyet tenylegesen tisztitani kell. | P1 | required-now | T2, T4, T7, T8, T9 |
| CS6 | `tests/v11/application/start/startCommandLaunchWorkspace.test.ts` | launch workspace authority unit proof | vitest | local unit validation seam | az explicit clone authority acceptance, a legacy clone fallback tiltasa, es a worktree legacy baseline kulon bizonyitast kap | P1 | required-now | T1-T4, T6 |
...
| runtime session claim authority semantics | resume claim synthetic clone authorityt is tud generalni `worktreePath + work_mode` alapjan | clone authority field csak explicit canonical source-bol maradhat/irhato; input-arg initial helper clone resumenel nem seedelhet clone authorityt; worktree fallback csak worktree mode-ra ervenyes; helper-level kind inference csak persisted `workspacePath` mellett elfogadhato | `worktreePath`, `tmuxSessionName` | explicit `workspacePath`, explicit `workspaceKind` | narrowing, fail-closed hardening | P1 | required-now |
| start contract corpus clone scenarios | `clone_not_activated`, `clone_not_activated_resume` fail-closed | activation success scenariok + authority-missing fail-closed scenariok + retained `clone_state_not_startable`; a historic alias scenario/file nev onmagaban nem jelenthet retained preflight reject semantics-et | `scenario`, `expected.status`, `expected.reasonCode` | `resumeState` | breaking test corpus, plan-owned | P1 | required-now |
| local start reason-code outputs | clone activation előtti retained local start error code | local Phase 2A start authority-failure outputs `START_LAUNCH_WORKSPACE_UNAVAILABLE`, precedence lane `START_STATE_NOT_STARTABLE`, es a retired `WORKSPACE_MODE_CLONE_NOT_ACTIVATED` negativ assertionnel kizart a local start/test corpusban | `expected.reasonCode` | `expected.status` | breaking public contract at local start surface | P1 | required-now |
...
| fresh clone bootstrap hianyzo canonical authorityval | bootstrap workspace port | throw | fresh cleanup + runtime session cleanup retained baseline | `START_LAUNCH_WORKSPACE_UNAVAILABLE` | error | P1 | required-now |
...
| local clone start Phase 2A authority-failure / precedence assertions | API + launch workspace + contract harness | throw | negative assertion: local start pathon `WORKSPACE_MODE_CLONE_NOT_ACTIVATED` nem terhet vissza sem fresh, sem resume, sem contract-corpus authority-failure lane-ben | `START_LAUNCH_WORKSPACE_UNAVAILABLE` vagy `START_STATE_NOT_STARTABLE` | error | P1 | required-now |
...
| T2 | fresh clone hianyzo bootstrap authority fail-closed cleanup-pal | `work_mode=clone`, bootstrap result nem ad ervenyes canonical authorityt | `startBubble(...)` fresh pathon futna | nincs tmux launch, a fresh cleanup/runtime session cleanup retained marad, a hiba authority-missing familyben marad, es a local start output explicitten nem `WORKSPACE_MODE_CLONE_NOT_ACTIVATED` | P1 | required-now | `tests/v11/application/start/startCommandLaunchWorkspace.test.ts`, `tests/core/bubble/startBubble.test.ts` |
...
| T4 | resume clone legacy fallback forbidden | resumable bubble, runtime session csak retained worktree pathot vagy `workspaceKind=clone` authority nelkul tart | `startBubble(...)` resume pathon futna | nincs tmux launch, nincs synthetic clone authority, a hiba `START_LAUNCH_WORKSPACE_UNAVAILABLE` marad, es a local start output explicitten nem `WORKSPACE_MODE_CLONE_NOT_ACTIVATED` | P1 | required-now | `tests/v11/application/start/startCommandLaunchWorkspace.test.ts`, `tests/core/bubble/startBubble.test.ts` |
...
| T7 | clone state-not-startable precedence retained | `work_mode=clone`, bubble state `FAILED` | public/local start export fut | `START_STATE_NOT_STARTABLE` marad a public hiba, nem activation-specifikus clone authority error, es explicit negative proof van arra, hogy itt sem ter vissza `WORKSPACE_MODE_CLONE_NOT_ACTIVATED` | P1 | required-now | `tests/core/bubble/startBubble.test.ts`, `tests/contracts/v11/start.contract.runner.ts`, `tests/contracts/v11/cases/start/start-clone-state-not-startable-v11.case.json` |
| T8 | start contract fresh clone activation corpus | contract case fresh clone explicit bootstrap authorityval | shared contract runner fut | a corpus start success kimenetet var el, nem `clone_not_activated` fail-closedot, es a historic alias case sem igazolhat retained local preflight rejectet | P1 | required-now | `tests/contracts/v11/start.contract.runner.ts`, `tests/contracts/v11/start.contract.test.ts`, `tests/contracts/v11/cases/start/start-clone-not-activated-v11.case.json` |
| T9 | start contract resume clone activation corpus | contract case resumable clone explicit persisted canonical authorityval tobb resumable stateben | shared contract runner fut | a corpus success kimenetet var el resume clone esetben is, mikozben a nem-startolhato clone state case retained marad, es egyik resume alias case sem terhet vissza `WORKSPACE_MODE_CLONE_NOT_ACTIVATED`-ra authority-failure lane helyett | P1 | required-now | `tests/contracts/v11/start.contract.runner.ts`, `tests/contracts/v11/start.contract.test.ts`, `tests/contracts/v11/cases/start/start-clone-not-activated-resume-v11.case.json`, `tests/contracts/v11/cases/start/start-clone-not-activated-resume-waiting-human-v11.case.json`, `tests/contracts/v11/cases/start/start-clone-not-activated-resume-ready-for-human-approval-v11.case.json`, `tests/contracts/v11/cases/start/start-clone-not-activated-resume-approved-for-commit-v11.case.json`, `tests/contracts/v11/cases/start/start-clone-not-activated-resume-committed-v11.case.json` |
...
2. [later-hardening] A contract corpus scenario/file neveit erdemes a Phase 2A utan mar activation-semantikara atnevezni, hogy a retained `clone_not_activated` token ne maradjon felrevezeto historic alias ott sem, ahol Phase 2A-ban tudatosan megtartottuk a path-stabilitast.
