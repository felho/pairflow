...
2. Direct non-authoritative `resolveBubbleFromWorkspaceCwd(...)` activation clone workspace rootra
...
| --- | --- | --- | --- | --- |
| `src/v11/shared/actorProtocol/actorEmitContext.ts` `ActorEmitContextSnapshot` | actor protocol wrappers (`pass`, `human_question`, `convergence`, `meta_review_result`), pass/converged/ask-human authoritative prep tests | additive | explicit `workspace_path` executable-root field bevezetese, retained `worktree_path` megtartasa traceabilityre | direct non-authoritative cwd activation Phase 2A |
| authoritative workspace prep consume (`pass` / `converged` / `ask-human`) | bubble-loop command orchestration family | additive / semantic-clarifying | authoritative context eseten `cwd` a `workspace_path`-ra all at, mikozben artifact pathok retained worktree pathon maradnak | operator read-model successor-only |

### 0b) Baseline Preservation (if applicable)

| Current Behavior | Preserve/Replace/Forbid | Required Proof | Priority | Timing |
| --- | --- | --- | --- | --- |
| actor wrapper dispatch `cwd: context.worktree_path` | replace | wrapper es prep tesztek bizonyitjak, hogy `cwd` `workspace_path`-ra all at | P1 | required-now |
...
| CS2 | `src/v11/application/actorProtocol/actorProtocolEmitters.ts` | `emitPassActorResultV11(...)` | `(input) -> Promise<ActorEmitResultV11 pass>` | wrapper dispatch `cwd` | a wrapper `cwd` a `context.workspace_path`, nem a retained `worktree_path` | P1 | required-now | T5 |
| CS3 | `src/v11/application/actorProtocol/actorProtocolEmitters.ts` | `emitHumanQuestionActorResultV11(...)` | `(input) -> Promise<ActorEmitResultV11 human_question>` | wrapper dispatch `cwd` | ask-human wrapper ugyanazt a canonical workspace rootot hasznalja | P1 | required-now | T6 |
| CS4 | `src/v11/application/actorProtocol/actorProtocolEmitters.ts` | `emitConvergenceActorResultV11(...)` | `(input) -> Promise<ActorEmitResultV11 convergence>` | wrapper dispatch `cwd` | converged wrapper canonical workspace rootot hasznal, stale/state guard retained marad | P1 | required-now | T7 |
| CS5 | `src/v11/application/actorProtocol/actorProtocolEmitters.ts` | `emitMetaReviewActorResultV11(...)` | `(input) -> Promise<ActorEmitResultV11 meta_review_result>` | wrapper dispatch `cwd` | meta-review submit wrapper canonical workspace rootot kap | P1 | required-now | T8 |
...
| authoritative wrapper cwd consume | wrapper-ek `cwd: context.worktree_path` | wrapper-ek `cwd: context.workspace_path` | `workspace_path` | `worktree_path` retained | non-breaking internal consume cutover | P1 | required-now |
| authoritative prep consume | authoritative reuse branch `cwd/worktreePath` worktree-path-rol epul | authoritative reuse branch `cwd` `workspace_path`-rol epul, retained bubblePaths változatlan | `workspace_path` authoritative contextban | `worktree_path` retained | non-breaking internal consume cutover | P1 | required-now |
...
| direct non-authoritative cwd invocation | `resolveBubbleFromWorkspaceCwd` | fallback | retained current worktree baseline | retained existing command behavior | info | P1 | required-now |
...
| T2 | actor context legacy same-authority worktree fallback | runtime session explicit `workspacePath` nelkul, no-split worktree baseline | actor context materializalodik | a snapshot `workspace_path`-ja a megengedett same-authority fallbackot hordozza | P1 | required-now | `tests/v11/application/actorProtocol/emitActorProtocolV11.test.ts` |
...
| T4 | actor context clone fallback forbidden | runtime session csak clone-only legacy worktree fallbackot adna | actor context materializalodna | nincs silent worktree truth, wrapper fail-closed marad | P1 | required-now | `tests/v11/application/actorProtocol/emitActorProtocolV11.test.ts` |
| T5 | pass authoritative workspace consume | implementer vagy reviewer wrapper authoritative contexttal fut | `emitPass...` / `preparePassWorkspaceContext(...)` fut | wrapper `cwd` es authoritative prep `cwd` a `workspace_path`, mikozben retained artifact path bubble worktree marad | P1 | required-now | `tests/v11/application/actorProtocol/emitActorProtocolV11.test.ts`, `tests/v11/application/pass/passWorkspaceContextPreparation.test.ts` |
| T6 | ask-human authoritative workspace consume | authoritative ask-human wrapper input | `emitAskHuman...` / `prepareAskHumanWorkspaceContext(...)` fut | az authoritative execution root canonical workspace, de a retained worktree traceability el | P1 | required-now | `tests/v11/application/actorProtocol/emitActorProtocolV11.test.ts`, `tests/v11/application/askHuman/askHumanWorkspaceContextPreparation.test.ts` |
| T7 | converged authoritative workspace consume | reviewer authoritative context es persisted running state | `emitConverged...` / `prepareConvergedRouting(...)` fut | canonical workspace consume tortenik, stale/state reread baseline retained marad | P1 | required-now | `tests/v11/application/actorProtocol/emitActorProtocolV11.test.ts`, `tests/v11/application/converged/convergedRoutingPreparation.test.ts` |
| T8 | meta-review wrapper canonical workspace consume | canonical meta-reviewer authority aktiv | `meta_review_result` wrapper fut | `submitMetaReviewResult(...)` canonical workspace `cwd`-t kap, nem retained `worktree_path`-ot | P1 | required-now | `tests/v11/application/actorProtocol/emitActorProtocolV11.test.ts` |
...
2. [later-hardening] A Phase 2A activation taskban erdemes kulon bizonyitani, hogy a direct non-authoritative cwd resolution clone workspace rootrol is helyesen talalja meg a bubble authorityt.

## Hardening Backlog (Optional)

| ID | Item | Layer | Priority | Timing | Source | Proposed Action |
| --- | --- | --- | --- | --- | --- | --- |
| H1 | direct non-authoritative workspace resolution clone-topology consume-ja meg nincs lezarva | L2 | P2 | later-hardening | Phase 1D successor boundary | lezarni Phase 2A activation taskban |
