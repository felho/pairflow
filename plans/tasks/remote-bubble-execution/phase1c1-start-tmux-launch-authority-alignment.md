...
3. A runtime session record mar kepes canonical `workspacePath` / `workspaceKind` authorityt hordozni, de a start/tmux launch path ezt ma meg nem tekinti authoritative consume inputnak.
4. Emiatt a Phase 2A local clone-topology activation tovabbra sem nyithato meg, mert a critical start/tmux consume reteg meg nem valtott at a canonical authority chainre.
...
1. fresh es resume startban a tmux session/pane launch ugyanarra a canonical workspace authorityra uljon, amelyet az elozo fazis eloallitott,
...
1. Business invariant: egy start attempt alatt minden tmux launch consumer ugyanazt a canonical workspace identityt hasznalja `cwd`-hez es command-guidancehoz; nem lehet kulon status-pane, implementer/reviewer/meta pane vagy kickoff/startup prompt eltero workspace truthon.
...
3. Read-path rule: a tmux launch consume az explicit launch inputon vagy egy ugyanazon authority chainen belul dolgozo consume helperen keresztul olvashat canonical workspace pathot; a leaf consumer kozvetlen `context.resolved.bubblePaths.worktreePath` olvasasa tiltott a cutoveren atesett pontokon.
4. Forbidden fallback:
   - a static `bubblePaths.worktreePath` kozvetlen leaf fallbackkent,
   - operator status/read-model vagy lifecycle result felol visszaolvasott path,
   - reviewer-context vagy runtime-delivery consumer source,
   - tmux/session metadata vagy status wording alapjan rekonstrualt workspace truth.
5. Allowed resolution path:
   ...
   - ha egy surface-en a static bubble path tovabbra is szukseges nem-authoritative referenciahoz, azt kulon neven kell hordozni; nem keverheto a launch `cwd` truth-tal.
...
1. A fresh es resume start launch input explicit canonical workspace consume-ja.
2. A tmux session/pane `cwd` consume atallitasa canonical workspace authorityra.
3. A status pane command es a startup/resume/kickoff prompt builders workspace-guidance inputjanak canonical consume-ra allitasa.
4. A start launch call-siteok olyan additiv vagy egyertelmu interface-alakitasa, amely szetvalasztja a canonical launch workspace truth-ot a retained static bubble-root referenciatol, ha ugyanazon seam ma mindkettot `worktreePath` neven keveri.
5. A fenti consume szabalyok tesztjei fresh, resume es missing-authority helyzetekben.
...
3. A worktree-mode retained path csak explicit, non-authoritative referencia lehet; nem lehet implicit leaf fallback.
...
|---|---|---|---|---|
| Business invariant | Minden start/tmux launch consumer ugyanazt a canonical workspace identityt hasznalja. | Nem maradhat kulon `cwd` truth a tmux sessionnek, a pane splitnek es a startup/kickoff prompt guidance-nak. | P1 | required-now |
...
| Forbidden fallback | Static bubble path, operator read-model, reviewer-context es tmux metadata alapjan visszaolvasott truth tiltott. | Hianyzo canonical input eseten fail-closed launch, nincs masodlagos leaf fallback. | P1 | required-now |
...
|---|---|---|---|---|---|---|---|---|
| CS1 | `src/v11/application/start/startCommandApi.ts` | start orchestration launch handoff | `startBubble(...)` orchestration entry -> result/throw | `runStartFlow(...)` caller seam | a start orchestration explicit canonical launch workspace inputot ad tovabb a fresh/resume launch flow fele; a result payload es lifecycle metadata nem valik launch truth source-sza | P1 | required-now | T1, T2, T4 |
| CS2 | `src/v11/application/start/startCommandFlows.ts` | fresh/resume launch input assembly | `runFreshStartFlow(...)`, `runResumeStartFlow(...)` | tmux launch call elotti seam | a fresh es resume flow explicit canonical workspace launch inputot allit elo, es ha ez hianyzik, meg a tmux launch elott fail-closed hibara fut | P1 | required-now | T1, T2, T3 |
| CS3 | `src/v11/application/start/startCommandTmuxLaunch.ts` | launch leaf consume | `launchFreshTmuxSession(...)`, `launchResumeTmuxSession(...)` | start/tmux leaf seam | a leaf launch code mar nem nyul kozvetlenul `context.resolved.bubblePaths.worktreePath`-hoz authoritative consume celra; a status command, agent commandok, kickoff/startup prompt inputok ugyanazt a canonical launch workspace erteket kapjak | P1 | required-now | T1, T2, T5 |
| CS4 | `src/v11/application/start/startCommandPrompts.ts`, `src/v11/application/start/startCommandImplementerPrompts.ts` | fresh startup/status prompt builders | existing prompt/status builders | status pane command es fresh startup prompt seam | a command-guidance es workspace display/cwd input canonical launch workspace authorityrol jon; ha retained static bubble-root referencia kell, az kulon, nem-authoritative mezoneven marad | P1 | required-now | T1, T5 |
| CS5 | `src/v11/application/start/startCommandResumePrompts.ts`, `src/v11/application/start/startCommandResumeImplementerPrompt.ts`, `src/v11/application/start/startCommandResumeKickoffMessages.ts`, `src/v11/application/start/startCommandResumeKickoffMessageBuilders.ts`, `src/v11/application/start/startCommandResumeFlowPreparation.ts` | resume startup/kickoff launch consume | existing resume prompt + kickoff builders | resume tmux startup guidance seam | a resume startup es kickoff messagek ugyanazt a canonical launch workspace inputot kapjak, amelyet a tmux launch is hasznal; nincs kozvetlen static bubble-path leaf consume | P1 | required-now | T2, T5 |
| CS6 | `src/v11/shared/ports/tmuxSessions.ts` | tmux launch port contract | `LaunchBubbleTmuxSessionInput` -> `LaunchBubbleTmuxSessionResult` | shared tmux port | a port cwd semantics egyertelmuen canonical launch workspace authorityt jelol; ha bubble-root referencia is kell, az additiv es kulon nevezett marad | P1 | required-now | T5, T6 |
| CS7 | `src/v11/infrastructure/channel/tmux/tmuxManager.ts`, `src/v11/infrastructure/channel/tmux/tmuxManagerSessionLayout.ts` | tmux session/pane cwd consume | existing launch/session-layout exports | tmux infra cwd seam | a new-session es split-window `-c` consume ugyanazt a canonical launch workspace erteket hasznalja minden pane-nel; nincs hidden static bubble-path fallback | P1 | required-now | T6 |
| CS8 | `tests/core/bubble/startBubble.test.ts`, `tests/core/runtime/restartRecovery.test.ts`, `tests/core/runtime/tmuxManager.test.ts`, `tests/contracts/v11/start.contract.runner.ts`, `tests/contracts/v11/start.contract.test.ts`, `tests/v11/application/start/startCommandOrchestration.test.ts` | launch consume regression tests | unit/integration/contract | validation surface | explicit bizonyitas kell arra, hogy a launch consumers a canonical workspace authorityt hasznaljak fresh/resume pathon is, es hiany eseten fail-closed maradnak | P1 | required-now | T1-T6 |
...
| Tmux launch port cwd semantics | egyetlen `worktreePath` mezore ul a session/pane `cwd` | a shared port egyertelmuen canonical launch workspace `cwd`-t jelent, vagy additiv mezokkel szetvalik a canonical `cwd` es a bubble-root reference | canonical `cwd` path | bubble-root reference only if separately named and not used as launch truth | shared-contract alignment required in-scope consumerskel | P1 | required-now |
...
2. Ha a jelenlegi `worktreePath` mezon ugyanrejtve ket fogalom utazik, a task additiv szetvalasztast preferal.
3. Fresh startban az explicit launch workspace input ugyanazon attempt authority chainjebol jojjon, ne operator/result surface-bol.
4. Resume pathon ugyanaz a helper vagy assembly nyelv legyen ervenyes, mint fresh pathon; a kulonbseg csak az authority source-ja lehet, nem a leaf consumer fallbackja.
...
| leaf launch consumer static bubble pathra esne vissza | launch/prompt builder consume | throw | nincs fallback; explicit invariant failure vagy normalizalt start hiba | existing start error normalization retained | error | P1 | required-now | T3, T5 |
| shared tmux port nem tudja megkulonboztetni a canonical `cwd`-t a bubble-root referenciatol | tmux port contract | throw vagy additive contract refinement | nincs silent overload | existing contract-test failure surface retained | error | P1 | required-now | T5, T6 |
...
| must-not-use | lifecycle result payload vagy event metadata mint launch truth source | P1 | required-now |
...
|---|---|---|---|---|---|---|---|
| T1 | fresh tmux launch consumes canonical workspace authority | `work_mode=worktree`, fresh start, canonical workspace launch input explicit | `startBubble(...)` lefut a fresh launchig | a `launchBubbleTmuxSession` input, a status pane command es a startup promptok ugyanazt a canonical workspace erteket kapjak; nincs leaf static bubble-path consume authoritative szerepben | P1 | required-now | `tests/core/bubble/startBubble.test.ts` |
| T2 | resume tmux launch consumes same-authority canonical path | resumable worktree bubble, runtime authority mar rendelkezik canonical workspace identityvel | `startBubble(...)` resume pathon lefut | a resume tmux launch es resume prompt/kickoff surfaces canonical launch workspace inputot kapnak; a leaf code nem olvas kozvetlen `bubblePaths.worktreePath`-ot authoritative consume-kent | P1 | required-now | `tests/core/runtime/restartRecovery.test.ts`, `tests/core/bubble/startBubble.test.ts` |
| T3 | missing canonical launch authority fails closed before tmux launch | fresh vagy resume pathon a launch consume input nem oldhato fel | a start launch reszhez er | a start hibat dob a tmux launch elott, es nincs silent fallback a static bubble pathra | P1 | required-now | `tests/core/bubble/startBubble.test.ts` |
...
| T5 | shared launch contract preserves clarity when canonical `cwd` es bubble-root reference elvalik | a launch port/tests intentionalisan kulon canonical es static path fixture-rel futnak | start/tmux launch builder path assembly es contract tests lefutnak | explicit bizonyitas van arra, hogy a canonical launch `cwd`-t a correct input hajtja, es a kept bubble-root reference nem csuszik vissza launch truthkent | P1 | required-now | `tests/contracts/v11/start.contract.test.ts`, `tests/v11/application/start/startCommandOrchestration.test.ts` |
| T6 | tmux manager and pane layout use canonical cwd for all panes | a launch port canonical launch workspace pathot kap | `launchBubbleTmuxSession(...)` fut | a `new-session` es minden `split-window -c` ugyanazt a canonical workspace cwd-t hasznalja | P1 | required-now | `tests/core/runtime/tmuxManager.test.ts` |

### 7) Shared Contract Compatibility

| Contract | Current Consumers | Additive or Breaking | Alignment Ownership | Priority | Timing |
|---|---|---|---|---|---|
| `LaunchBubbleTmuxSessionInput` cwd semantics | `startCommandTmuxLaunch.ts`, `tmuxManager.ts`, `tmuxManagerSessionLayout.ts`, kapcsolodo start/tmux tesztek | additive preferalt, ha canonical `cwd` es bubble-root reference kulon jelentessel marad | teljesen ebben a taskban zarando, mert minden jelenlegi consumer in scope | P1 | required-now |
...
|---|---|---|---|---|---|---|
| H1 | A Phase 1C2 runtime delivery/reviewer-context consume ugyanazt a canonical workspace helper-szokincset hasznalja, mint a Phase 1C1 launch consume | L2 | P2 | later-hardening | phase split boundary | Phase 1C2-ben lezarni |
