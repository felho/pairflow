...
### Canonical vs Diagnostic Truth Split

1. Canonical ack truth forrasa ebben a fazisban kizarolag az explicit typed delivery/launch ack producer seam, az `execution_id` + `handoff_id` + `round` targeteles, es az ezekhez kotott persisted observation.
2. A retained tmux marker, pane activity, session nev, capture output es operator-visible warning/status szoveg tovabbra is csak diagnostics/observability input vagy projection lehet; ezek legfeljebb a mar meglevo typed ack observation korul adhatnak operator contextet.
3. A meta-review runtime delivery stored/projection seam current-tree ownershipe ebben a fazisban csak annyi, hogy losslessen tukrozze vagy explicit non-canonicalkent jelezze a typed ack observationt; nem vezethet be kulon meta-review runtime vocabularyt vagy authority forrast.
4. Ha diagnostics surface es canonical ack observation elcsuszik egymastol, a consumernek a canonical typed ack truthot kell elonyben reszesitenie, es a driftet legfeljebb operator diagnosticskent szabad megjelenitenie.
...
| Phase | Owns | Must Not Reopen |
|---|---|---|
| `E2` | typed delivery ack contract, typed launch ack contract, canonical ack vocabulary closure, direct current-tree fallout (`watchdog`, `restart/recovery`, existing status/warning wording, retained stored runtime-delivery projection) | implementer pilot activation, reviewer/meta-reviewer rollout, retained adapter cleanup, broad diagnostics redesign |
| `E3` | implementer pilot activation az `E2` altal lezart typed ack boundaryn | canonical ack vocabulary ujranyitasa, reviewer/meta-reviewer rollout, retained cleanup |
| `E4` | reviewer/meta-reviewer rollout, retained adapter/operator cleanup, maradek diagnostics cleanup | `E1` execution authority foundation vagy `E2` canonical ack contract ujradefinialasa |
...
|---|---|---|---|---|
| `EmitTmuxDeliveryNotificationResult` | pass/converged/request-rework/watchdog/kickoff flows | breaking internal shared contract | `delivered:boolean` helyett explicit typed delivery ack result vagy ekvivalens split-contract | `E2` zarja a contractot; `E3-E4` legfeljebb a lezart contract consume/cleanup rolloutjat vegezheti |
| `EmitTmuxDeliveryNotificationPort` | delivery helper layers | breaking internal port | minden consume hely explicit typed ackot kezeljen | `E2` zart consume parity a current-tree feluleteken; `E3-E4` csak kesobbi rollout/cleanup |
| `LaunchBubbleTmuxSessionResult` | start/resume launch flow | breaking internal port | explicit launch ack / failure semantics threadelese | `E3` az itt lezart launch boundaryt hasznalja pilot activationhoz; nem definial uj launch truthot |
| meta-review runtime delivery stored state | meta-review gate, status, inspect/read path | breaking internal diagnostics contract | `confirmed|uncertain|failed` canonical source-of-truthkent megszunik vagy lossless typed projectionne valik | `E4` csak retained diagnostics cleanupot vegezhet; a canonical-vs-diagnostics split mar `E2`-ben zarul |
...
| status/read-model runtime projection | `bubble status` text/table and list fallout | breaking diagnostics projection | execution-bound typed ack projection jelenik meg pane-helyettesites nelkul | `E2` owns only the direct typed-ack fallout on existing surfaces; `E4` only non-critical cleanup/copy follow-up |
...
2. `R2_EXECUTION_TARGETING`: canonical ack observation csak explicit `observed_for_execution_id`, `observed_for_handoff_id`, `observed_for_round` es `observed_at` mellett ervenyes.
...
5. `R5_DUPLICATE_INTERIM_GUARD`: duplicate masodik signal nem hozhat letre masodik `accepted` vagy `running` truthot ugyanarra a current executionre; ez required-now, de csak `P2` interim guard ownership.

### 0d) Current-Tree Ownership And Proof Limits

1. `E2` current-tree ownershipe csak azokra a seam-ekre terjed ki, ahol a tree ma is canonical runtime delivery/launch truthot gyart vagy annak kozvetlen falloutjat tarolja/projektalja.
2. A `start/resume` call-site-ok es tesztek ebben a taskban csak a launch-boundary contract closuret bizonyitjak; implementer actor-path activation vagy pilot-orchestration ownership tovabbra is `E3`.
3. A meta-review runtime delivery persistence/projection seam ebben a taskban diagnostics-only retained consumer: targetelesi parityt es non-canonical mismatch kezelest kell bizonyitania, nem meta-review runtime rolloutot.
4. A status/CLI wording proof ebben a taskban csak a no-pane-inference es typed-ack terminology szabalyokat zarja le a current surfacesen; removed vagy retained operator surface cleanup nem `E2` ownership.
5. A duplicate-delivery proof ebben a taskban csak interim invariant proof: nem teljes retry/replay UX policy, es nem nyitja ujra a broad suppression ownershipet.

### 1) Call-site Matrix

| ID | File | Function/Entry | Exact Signature (args -> return) | Insertion Point | Expected Behavior | Priority | Timing | Evidence |
|---|---|---|---|---|---|---|---|---|
| CS1 | `src/v11/shared/delivery/tmuxDeliveryContract.ts` | `EmitTmuxDeliveryNotificationResult`, failure/status types | type shapes | retained delivery contract seam | A canonical delivery result typed ack vocabularyt hordozzon; `delivered:boolean` onmagaban megszunik. | P1 | required-now | `tests/core/runtime/tmuxDelivery.test.ts`, `tests/v11/application/converged/convergedExecution.test.ts` |
| CS2 | `src/v11/shared/ports/tmuxDelivery.ts` | exported delivery port types | port types | shared port seam | Port consumer mindenhol az uj typed resultot olvassa. | P1 | required-now | `tests/v11/shared/delivery/implementerHandoffDelivery.test.ts`, `tests/v11/application/converged/convergedExecution.test.ts`, `tests/core/bubble/watchdogBubble.test.ts` |
| CS3 | `src/v11/infrastructure/channel/tmux/tmuxDeliveryRuntime.ts` | `attemptTmuxDelivery`, `createDeliveryFailureResult` | infra delivery attempt -> result | retained delivery producer | Marker-confirmation vagy tmux-failure mar typed delivery ack observationne/projectionne fordul. | P1 | required-now | `tests/core/runtime/tmuxDelivery.test.ts` |
| CS4 | `src/v11/shared/ports/tmuxSessions.ts` | `LaunchBubbleTmuxSessionResult` | tmux launch port result | launch result contract | Fresh/resume launch explicit launch ack / failure semanticsat hordozzon. | P1 | required-now | `tests/core/runtime/tmuxManager.test.ts`, `tests/core/runtime/restartRecovery.test.ts` |
| CS5 | `src/v11/infrastructure/channel/tmux/tmuxManager.ts` | `launchBubbleTmuxSession` | launch input -> launch result | retained session launch producer | Session launch resultnek explicit launch-boundary statuszt kell adnia, nem csak session nevet. | P1 | required-now | `tests/core/runtime/tmuxManager.test.ts` |
| CS6 | `src/v11/application/start/startCommandTmuxLaunch.ts` | `launchFreshTmuxSession`, `launchResumeTmuxSession` | start context -> tmux launch result | start launch boundary | A start flow typed launch ackot threadel, nem csak `sessionName`-et. | P1 | required-now | `tests/core/runtime/tmuxManager.test.ts`, `tests/core/runtime/restartRecovery.test.ts` |
| CS7 | `src/v11/application/start/startCommandFlows.ts`, `src/v11/application/start/startCommandContract.ts` | fresh/resume start orchestration and result contracts | start input -> `StartBubbleResult` | start consume seam | Start/resume failure/success semantics explicit launch ack boundaryra epuljenek. | P1 | required-now | `tests/core/runtime/restartRecovery.test.ts`, `tests/cli/bubbleStartCommand.test.ts` |
| CS8 | `src/v11/application/converged/convergedGateDelivery.ts` | `buildConvergedDelivery`, aggregate reason resolver | delivery results -> CLI-facing result | converged delivery aggregation | Aggregate result explicit typed delivery outcome alapjan alljon, ne boolean failed-counton. | P1 | required-now | `tests/cli/convergedDeliveryWarning.test.ts`, `tests/v11/application/converged/convergedExecution.test.ts` |
| CS9 | `src/v11/shared/delivery/implementerHandoffDelivery.ts` | `shouldRetryImplementerHandoffDelivery`, `executeImplementerHandoffDelivery` | delivery input -> result | handoff retry seam | Retry policy csak explicit retryable ack-unavailable/failure esetre aktivalodjon. | P1 | required-now | `tests/v11/shared/delivery/implementerHandoffDelivery.test.ts`, `tests/core/bubble/watchdogBubble.test.ts`, `tests/core/runtime/tmuxDelivery.test.ts` |
| CS10 | `src/v11/shared/kickoff/kickoffValidatedExecutionDelivery.ts` | `mapKickoffResultDelivery`, fallback builders | validation + envelope -> kickoff delivery projection | kickoff delivery seam | Kickoff consumer mar typed delivery resultot adjon vissza. | P1 | required-now | `tests/core/runtime/tmuxDelivery.test.ts` |
| CS11 | `src/v11/application/watchdog/watchdogPendingReworkIntent.ts` | `maybeApplyPendingReworkIntent` | watchdog input -> watchdog result | deferred rework consume seam | Pending rework intent csak explicit accepted-equivalent delivery mellett alkalmazhato; unavailable/failure explicit visszajelzest ad. | P1 | required-now | `tests/core/bubble/watchdogBubble.test.ts` |
| CS12 | `src/v11/shared/metaReviewGate/metaReviewGateTypes.ts`, `src/v11/shared/metaReviewGate/metaReviewGateApplyPersistence.ts` | runtime delivery observation types/persistence | gate run -> persisted snapshot | meta-review retained diagnostics seam | Meta-review runtime delivery persisted recordje az uj typed ack boundaryra alignaljon, `execution_id` target mezovel. | P1 | required-now | `tests/v11/shared/metaReview/metaReviewSnapshot.test.ts`, `tests/v11/shared/state/stateSchema.test.ts` |
| CS13 | `src/types/bubble.ts`, `src/v11/shared/state/stateSchemaMetaReviewRuntime.ts`, `src/v11/infrastructure/state/stateSnapshotInspection.ts`, `src/v11/shared/metaReview/metaReviewSnapshot.ts` | persisted runtime delivery state, validation, inspect normalization | snapshot state -> validated/inspectable state | persisted diagnostics seam | Stored meta-review runtime delivery explicit typed statuszt es `observed_for_execution_id` parityt hordozzon. | P1 | required-now | `tests/v11/shared/metaReview/metaReviewSnapshot.test.ts`, `tests/v11/shared/state/stateSchema.test.ts`, `tests/cli/bubbleStatusCommand.test.ts` |
| CS14 | `src/v11/shared/status/statusCommandViewProjection.ts`, `src/v11/application/status/statusCliTextRenderer.ts`, `src/v11/application/status/statusCliTableRenderer.ts` | status projections/renderers | state -> view text/table | diagnostics fallout | Status explicit typed ack state-et jelenitsen meg, pane activitytol kulon. | P1 | required-now | `tests/cli/bubbleStatusCommand.test.ts` |
| CS15 | `src/cli/index.ts` | pass/converged/request-rework warning writers | command result -> stderr text | CLI warning seam | Warning text delivery/launch ack terminologyval beszeljen, ne "active pane confirmed" framinggel. | P1 | required-now | `tests/cli/passAutoConvergeWarning.test.ts`, `tests/cli/convergedDeliveryWarning.test.ts`, `tests/cli/requestReworkDeliveryWarning.test.ts` |
...
| Status/CLI text | terminology es projection alignment | broad UX redesign | diagnostics-only change; broad UX cleanup/rescoping `E4` ownership | P1 | required-now |
...
1. A tesztmatrix `E2`-owned bizonyiteka csak a canonical ack boundaryra es annak kozvetlen current-tree falloutjara vonatkozik.
2. `T4` launch-boundary proof, nem implementer pilot activation proof.
3. `T6` diagnostics/parity proof, nem meta-review rollout proof.
4. `T8` interim guard proof, nem full duplicate policy closure.

| ID | Scenario | Given | When | Then | Priority | Timing | Evidence |
|---|---|---|---|---|---|---|---|
| T1 | pass/converged/request-rework warnings typed ack contractot hasznalnak | delivery result mar nem boolean-only | CLI command result renderelodik | a warning szoveg nem "active pane confirmed" framinget hasznal, es explicit ack-outcome terminologyt ad | P1 | required-now | `tests/cli/passAutoConvergeWarning.test.ts`, `tests/cli/convergedDeliveryWarning.test.ts`, `tests/cli/requestReworkDeliveryWarning.test.ts` |
| T2 | explicit delivery failure nem lep tovabb watchdogban | pending deferred rework intent + unavailable/rejected delivery outcome | watchdog fut | az intent pending marad es explicit ack-failure reason jelenik meg | P1 | required-now | `tests/core/bubble/watchdogBubble.test.ts` |
| T3 | explicit accepted delivery alkalmazhatja a pending reworkot | pending deferred rework intent + accepted delivery outcome | watchdog fut | az intent alkalmazodik, a flow tovabblep | P1 | required-now | `tests/core/bubble/watchdogBubble.test.ts` |
| T4 | start/resume explicit launch ackot ad | fresh vagy resumable bubble delivery-failure recovery nelkul | start flow fut | a returned launch result explicit `running` vagy `failed_to_start` boundaryt ad, nem csak session nevet | P1 | required-now | `tests/core/runtime/tmuxManager.test.ts` |
| T5 | pane activity nem helyettesiti a canonical ackot | pane mutat activityt, de explicit ack nincs | status/warning path renderel | nincs implicit running/accepted claim, legfeljebb diagnostics jelenik meg | P1 | required-now | `tests/cli/bubbleStatusCommand.test.ts`, `tests/cli/passAutoConvergeWarning.test.ts`, `tests/cli/convergedDeliveryWarning.test.ts`, `tests/cli/requestReworkDeliveryWarning.test.ts` |
| T6 | meta-review runtime delivery targetelt marad | persisted runtime delivery mas execution_id/handoff/round parra mutat | status/inspect snapshot olvasodik | az observation inactive/non-canonical lesz, es nem ir felul canonical ack truthot | P1 | required-now | `tests/cli/bubbleStatusCommand.test.ts`, `tests/v11/shared/metaReview/metaReviewSnapshot.test.ts`, `tests/v11/shared/state/stateSchema.test.ts` |
| T7 | restart recovery explicit ack failure utan is uj current executionnel routeolhato | delivery failure vagy missing pane binding tortent | restart/recovery lefut | stale ack nem rebindolodik, uj executionnel marad routeolhato a flow | P1 | required-now | `tests/core/runtime/restartRecovery.test.ts` |
| T8 | duplicate delivery nem hoz letre masodik success truthot | ugyanaz a work item ketszer triggerelodik | delivery/launch boundary consume lefut | masodik signal explicit reject/no-op, de nem masodik accepted/running | P2 | required-now | `tests/core/runtime/tmuxDelivery.test.ts`, `tests/core/runtime/tmuxManager.test.ts` |
...
5. Az `E2` acceptance launch-oldali bizonyiteka a `start/resume` launch-boundary closureig terjed; implementer pilot activation vagy orchestration parity nem resze ennek a tasknak.
6. Az `E2` acceptance meta-review-oldali bizonyiteka a targetelesi parityt es a non-canonical diagnostics kezelest zarja le; teljes meta-review runtime rollout nem acceptance-feltetel.
7. Az `E2` acceptance duplicate-signal bizonyiteka csak az interim `no second success truth` invariansra terjed ki; retry/replay UX vagy teljes suppression policy closure tovabbra is kesobbi fazis.
