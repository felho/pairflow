...
| CS2  | `src/core/state/transitions.ts`                                                                                              | transition graph                 | `canTransition(from, to) -> boolean` and `getAllowedTransitions(from)`            | directed transitions + active runtime states     | Support canonical Phase 2 transitions: `READY_FOR_APPROVAL -> META_REVIEW_RUNNING`, `READY_FOR_APPROVAL -> READY_FOR_HUMAN_APPROVAL` (sticky-gate bypass edge), `META_REVIEW_RUNNING -> RUNNING\|READY_FOR_HUMAN_APPROVAL`, `READY_FOR_HUMAN_APPROVAL -> RUNNING\|APPROVED_FOR_COMMIT` | P1       | required-now | T1,T2,T9                 |
