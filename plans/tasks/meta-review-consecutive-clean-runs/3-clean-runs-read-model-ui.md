...
1. Inspected entrypoints / call-sites:
   ...
   - `updateBubbleReviewPolicy` already accepts `meta_review_consecutive_clean_runs_required` in patches, while `buildSharedUiReviewPolicyPatch` still maps only one severity value to both severity fields.
...
1. Extend review-policy runtime/read-model types to include `meta_review_consecutive_clean_runs_required`.
...
2. Impacted contracts:
   - status/list/UI read-model payload shape
...
| Conflict response includes new field | UI normalizes and displays the current backend truth | P2 |
| Remote status payload includes new fields | parser validates and projects them | P2 |

### 2) Implementation Constraints

1. Reuse `normalizeBubbleReviewPolicy` for requirement defaults.
...
4. Keep `reviewer_blocking_min_severity` behavior explicit:
   - if the existing shared UI patch intentionally keeps reviewer blocking and meta-review severity aligned, preserve that behavior for `P1`/`P2`/`P3` unless the implementation proves the UI now owns a separate control.
   - `P3+2` must at least write meta-review threshold `P3` and requirement `2`.
...
| --- | --- | --- | --- | --- |
| `BubbleReviewPolicyRuntimeView` | status/list/UI/API tests | additive | add requirement field | docs in Task 4 |
...
1. Status/list/UI summaries expose configured clean-run requirement and current streak where policy/meta-review detail is already visible.
2. Legacy missing fields normalize to requirement `1` and streak `0`.
3. UI preset mapping is exact for `P1`, `P2`, `P3`, and `P3+2`.
4. Unsupported backend pairs project to custom/unsupported UI state.
5. UI update path writes both severity and requirement for selected presets.
6. Remote status payload validation covers the new fields.
7. Existing guarded/closed-runtime review-policy projection tests continue to pass.

## L2 - Implementation Plan

1. Add typed read-model fields for required clean runs and current clean streak in backend status/list contracts.
2. Extend `buildRuntimeAlignedReviewPolicyRuntimeView` or adjacent projection helpers to include `meta_review_consecutive_clean_runs_required`.
3. Extend status/list meta-review projection to include `consecutive_clean_runs`.
4. Extend remote status payload parser/validator and tests for the additive fields.
5. Add a typed UI quality-preset map and exact projection helper.
6. Update UI state normalization to accept requirement/streak and preserve custom unsupported pairs.
7. Update `ActionBar` review-policy controls to show quality preset language and write exact patch pairs.
8. Update router/API tests and UI store/action tests for `P1`, `P2`, `P3`, `P3+2`, unsupported pair fallback, and conflict payload normalization.
9. Run:
   - `pnpm lint`
   - `pnpm typecheck`
   - `pnpm test tests/core/bubble/statusBubble.test.ts tests/core/bubble/listBubbles.test.ts tests/core/ui/updateBubbleReviewPolicyForUi.test.ts tests/v11/shared/reviewPolicy/updateBubbleReviewPolicy.test.ts tests/v11/infrastructure/executor/ssh/sshBubbleStatus.test.ts ui/src/state/useBubbleStore.test.ts ui/src/components/actions/ActionBar.test.tsx ui/src/lib/api.test.ts`
   - `pnpm build`
