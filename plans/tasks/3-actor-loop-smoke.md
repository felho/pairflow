...
Add one minimal runner-driven fake actor-loop smoke that reaches pass,
convergence, and meta-review approval through the canonical
`pairflow agent emit --kind ...` feedback surface. The smoke must prove actor
feedback ingestion, authority refresh, transcript/state progression, and
approval routing without a real LLM, real tmux interaction, real editor, real
browser, or direct state-file mutation by the fake actor.
...
2. Control model: Pairflow command dispatch, actor emit parsing, inbox and
   ...
   meta-review approval routing remain production-owned.
...
5. Keep the scenario minimal: one happy-path progression through pass,
   convergence, meta-review clean result, and approval-ready or approved
   terminal gate as exposed by current Pairflow lifecycle behavior.
6. Preserve fast deterministic execution suitable for regular PR use.
...
4. Multi-round review loops, failure-mode scenarios, request-rework flows, and
   full commit/merge lifecycle closure.
...
| Lifecycle progression | Pass, convergence, and meta-review approval progression are production-owned. | Pairflow transcript/state/inbox/registry artifacts. | State advances through the expected actor loop without direct fake writes. | Fail when production state or transcript does not match the scenario step. | Assertions read public status/read-model or fixture-visible production artifacts. |
...
| Scope boundary | This task ships actor-loop smoke only. | `tests/almostE2e/actorLoopSmoke.test.ts` and helper extensions. | UI action and Layer 3 coverage remain successor/deferred work. | Review failure if this task claims UI, HTTP, browser, or full close coverage. | Test names and assertions stay limited to actor-loop progression. |
...
3. It does not own UI action dispatch, HTTP transport, browser rendering,
   failure-mode matrices, or full approve/commit/merge closure.
...
|---|---|---|---|---|---|
| Maintainers can run a Phase 1 actor-loop smoke that proves public actor emit wiring through pass, convergence, and meta-review approval. | end_to_end | Root Vitest execution of `tests/almostE2e/actorLoopSmoke.test.ts` using the almost-e2e runner and public actor emit CLI. | Fixture repo, fake external adapters, runner scenario, actor-loop smoke test, and minimal helper exports. | Node/pnpm environment and fresh build when compiled CLI is invoked. | Passing focused smoke evidence plus normal validation evidence for changed tests/helpers. |
...
| Full close lifecycle | Deferred. | Commit/merge/approve happy path beyond the actor-loop approval gate stays post-Phase-1 unless the parent plan is refined. |
...
3. Add `tests/almostE2e/actorLoopSmoke.test.ts` that sets up an isolated
   fixture bubble, runs the scenario through public actor emit commands, and
   asserts refreshed status/read-model progression after each step.
...
3. preserving compiled/public entrypoint coverage would require modifying
   ...
   `target_write_files`;
4. the scenario cannot reach the claimed approval gate without changing
   lifecycle semantics;
