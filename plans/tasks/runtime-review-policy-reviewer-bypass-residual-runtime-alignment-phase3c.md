...
6. A shared `buildBubbleReviewPolicyRuntimeView(...)` helper current tree consume familyje ma harom downstream feluleten latszik:
   - `src/v11/shared/status/statusCommandViewBuilder.ts`
   - `src/v11/shared/list/listCommandEntryProjection.ts`
   - `src/v11/defaults/ui/updateBubbleReviewPolicyForUi.ts`
   Emiatt a Phase 3C ownership nem a shared helper contract ujranyitasa, hanem a downstream consume/read-path fail-closed alignmentja.
...
8. Emiatt a bypass Phase 3 teljes runtime closure-je csak akkor lesz vedheto, ha ezek a residual branches es a broad read-model parity consume pontok is ugyanarra a mar lezart activation truth-ra allnak ra.
...
3. a status/list/UI read-model consume family, valamint a status/list remote runtime-unavailable parity invalid/drifted vagy unavailable esetben se mutasson optimista bypass truthot,
4. cleanup/recovery, startup/resume es read-model parity fail-closed maradjon activation hianyaban,
5. a broad `status/list/UI` consume parity es a status/list remote runtime-unavailable parity csak existing consume surfacesen zaruljon le, remote executor/lifecycle redesign nelkul,
...
1. A parent plan explicit hatara mar rogzitett:
   ...
   - Phase 3C: residual runtime branches + broad `status/list/UI` fail-closed parity + remote runtime-unavailable parity
...
5. Why no further split is required:
   a touched surfaces negy consumer bucketbe esnek, de mind ugyanannak az orokolt Phase 3B activation truth-nak downstream consume/parity closure-i; nincs producer, schema vagy shared-contract reopen, igy a split tovabbi dokumentum-fragmentaciot okozna ugyanazon successor seam menten.
...
5. Read model consumers:
   status/list/UI review-policy consume family local invalid-state projectionje, valamint a remote runtime-unavailable snapshotok fail-closed projectionje a meglevo status/list surface-eken; lasd Current Codebase Check #6.
...
4. Why collapse is safe:
   ugyanannak a residual topology consume familynek a folytatasai, uj contract drift, producer-write vagy schema-risk nelkul; a broad read-model parity is ugyanennek az orokolt truth-nak fail-closed consume kovetkezmenye.
...
5. Status / list / UI local invalid-state view:
   invalid vagy driftelt local RUNNING state nem mutathat optimista `effective_loop_mode = "meta_only"` truthot ott, ahol a live activation authority mar nem bizonyitott; fail-closed guarded/full baseline vagy explicit diagnostics kell a meglevo status/list/UI projectionokban.
...
| CS2 | `src/v11/shared/metaReviewGate/metaReviewGateCurrentRunFinalization.ts` | meta-review finalize retained guards | `sticky_human_gate`, threshold fallback es finalize diagnostics retained guard marad; nem lephetnek elo bypass-topology authorityva aktiv bypass mellett sem, es a proofnak ezt kulon is le kell fednie | P1 | required-now |
...
| B6 | `requested=meta_only` + invalid/drifted local RUNNING | a status/list/UI consume family guarded/full baseline-t vagy explicit diagnosticsot mutat, optimista bypass truth nelkul |
...
| B8 | scope-boundedness / no hidden ownership expansion | nincs uj `review_policy` field/schema, shared-helper authority reopen, UI mutation, remote lifecycle redesign vagy broader startup/session ownership-bovites |
...
| T6 | invalid/drifted local broad read-model parity | a `status`, `list` es UI consume surfaces optimista bypass truth nelkul fail-closed vagy expliciten diagnosticos maradnak | P1 |
...
| T8 | residual alignment scope gate | nincs uj `review_policy` field/schema, shared-helper authority reopen, UI mutation vagy presentational scope; a start/resume resz kickoff/projection parityre korlatozodik | P1 |
...
| C5 | `statusCommandViewBuilder.ts`, `listCommandEntryProjection.ts`, `updateBubbleReviewPolicyForUi.ts`, `tests/core/bubble/statusBubble.test.ts`, `tests/core/bubble/listBubbles.test.ts`, `tests/core/ui/updateBubbleReviewPolicyForUi.test.ts` | B6 + T6 invalid/drifted local read-model fail-closed parity a meglevo projection/snapshot consume familyben; ebben a slice-ban a proofot a status/list/UI core-layer projection tesztek adjak, nem kulon v11-layer unit testek |
| C6 | `statusCommandViewBuilder.ts`, `listCommandEntryProjection.ts`, `tests/core/bubble/statusBubble.test.ts`, `tests/core/bubble/listBubbles.test.ts` | B7 + T7 remote runtime-unavailable snapshot parity a meglevo status/list projection surfacesen; ebben a slice-ban a proofot a remote-unavailable core-layer projection tesztek adjak, nem kulon v11-layer unit testek |
...
4. Activation hianyaban a residual branches es a hozzajuk tartozo status/list/UI read-model consume family fail-closed reviewer/full vagy guarded baseline-on marad.
...
7. Invalid/drifted local state mellett a status/list/UI read-model consume family nem mutathat optimista bypass truthot.
...
9. A task nem huz be UI mutation vagy presentational redesign scope-ot; a start/resume resz kickoff/projection parityre korlatozodik, a remote resz pedig status/list read-model fail-closed parityre.
