...
function resolveCanonicalMetaReviewReportJson(input: {
  ...
  return {
    ...
    findings_run_id: resolvedMetaReviewRunId,
    meta_review_run_id: resolvedMetaReviewRunId,
    findings_digest_sha256: isNonEmptyString(base.findings_digest_sha256)
      ? base.findings_digest_sha256.trim().toLowerCase()
      : null,
    findings_artifact_status: isNonEmptyString(base.findings_artifact_status)
      ? base.findings_artifact_status.trim()
      : isNonEmptyString(base.artifact_status)
        ? base.artifact_status.trim()
        : null,
    artifact_status: isNonEmptyString(base.findings_artifact_status)
      ? base.findings_artifact_status.trim()
      : isNonEmptyString(base.artifact_status)
        ? base.artifact_status.trim()
        : null,
