...
export function buildReviewerScoutExpansionWorkflowGuidance(): string {
  return [
    ...
    "4) `Final Consolidation`: deduplicate again across primary + expansion findings, calibrate severity, then emit one final reviewer output package that follows the deterministic section/field contract below."
  ].join(" ");
}
