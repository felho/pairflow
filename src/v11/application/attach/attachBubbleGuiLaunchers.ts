...
function inferFailureClassFromOutput(output: string): "launcher_unavailable" | "launcher_launch_failed" {
...
function reasonCodeFromFailureClass(
  failureClass: "launcher_unavailable" | "launcher_launch_failed"
): AttachBubbleReasonCode {
...
export function toLauncherFailureError(input: {
...
export function normalizeLauncherError(
  error: unknown,
  launcher: ExplicitAttachLauncher
): AttachBubbleError {
  if (
...
export async function launchGuiLauncher(
  launcher: GuiAttachLauncher,
