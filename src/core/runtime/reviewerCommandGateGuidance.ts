...
export const REVIEWER_COMMAND_GATE_REQ_A =
  "If review round is 1: do not use canonical convergence emit yet; use `pairflow agent emit --kind pass ...` and declare findings explicitly (`--finding` when findings exist, `--no-findings` only when the review is truly clean). Legacy `pairflow pass` and `pairflow converged` remain compatibility adapters only.";
export const REVIEWER_COMMAND_GATE_REQ_B =
  "If review round is at or above `severity_gate_round` and no blocker findings remain (document scope blocker requires `P0/P1` + `timing=required-now` + `layer=L1`): use `pairflow agent emit --kind convergence ...`; advisory findings must be passed as structured `--finding` entries and are limited to `P2/P3` on this path. Legacy `pairflow converged` remains a compatibility adapter only.";
...
export const REVIEWER_COMMAND_GATE_REQ_E =
  "If blocker findings remain under current scope policy, keep using `pairflow agent emit --kind pass ... --finding ...`. Legacy `pairflow pass --finding` remains a compatibility adapter only.";
...
export function buildReviewerFindingsPassInstruction(
  ...
  if (reviewArtifactType === "document") {
    return "Document scope: canonical `pairflow agent emit --kind pass ... --finding ...` for blockers is valid only when structured findings include strict qualifiers (`timing=required-now` + `layer=L1`). CLI `--finding` cannot encode these qualifiers, so unqualified `P0/P1` entries are advisory and should converge at/after `severity_gate_round` using `pairflow agent emit --kind convergence ... --finding ...` (`P2/P3` only), or plain canonical convergence when clean. Legacy `pairflow pass` / `pairflow converged` remain compatibility adapters only.";
  }

  return "If blocker findings (`P0/P1`) remain, first resolve `executionContext` via `pairflow bubble status --json`, then run `pairflow agent emit --kind pass --repo <repo> --bubble-id <id> --handoff-id <handoff-id> --summary ... --finding 'P1:...|artifact://...'` (repeatable; for P0/P1 include finding-level refs). Legacy `pairflow pass` remains a compatibility adapter only.";
}
