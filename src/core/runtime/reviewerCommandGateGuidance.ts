...
export const REVIEWER_COMMAND_GATE_REQ_A =
  "If review round is 1: do not use `pairflow converged`; use `pairflow pass` and declare findings explicitly (`--finding` when findings exist, `--no-findings` only when the review is truly clean).";
export const REVIEWER_COMMAND_GATE_REQ_B =
  "If review round is at or above `severity_gate_round` and no blocker findings remain (document scope blocker requires `P0/P1` + `timing=required-now` + `layer=L1`): use `pairflow converged`; advisory findings must be passed as structured `--finding` entries and are limited to `P2/P3` on this path.";
...
export const REVIEWER_COMMAND_GATE_REQ_E =
  "If blocker findings remain under current scope policy, keep using `pairflow pass --finding`.";
export const REVIEWER_COMMAND_GATE_REQ_F =
  "Routing matrix (copy-paste): blocker -> `pairflow pass --summary \"...\" --finding \"P1:Title|artifact://ref\"`; advisory-only (`P2/P3`) -> `pairflow converged --summary \"...\" --finding \"P2:Title|artifact://ref\"`; clean -> `pairflow converged --summary \"...\"` (no `--finding`).";
...
export function buildReviewerFindingsPassInstruction(
  ...
  if (reviewArtifactType === "document") {
    return "Document scope: `pairflow pass --finding` for blockers is valid only when structured findings include strict qualifiers (`timing=required-now` + `layer=L1`). CLI `--finding` cannot encode these qualifiers, so unqualified `P0/P1` entries are advisory and should converge at/after `severity_gate_round` using `pairflow converged --finding` (`P2/P3` only), or plain `pairflow converged` when clean.";
  }
