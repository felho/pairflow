...
| R-RAW-FIXTURES | A negative test's fixture path can silently erase the dimension under test (`JSON.stringify` flattens `-0` to `0`). Stage hostile values through channels that provably preserve them — raw text files. **WATCH status**: becomes a rule on a second occurrence. | log 2026-07-08 ch6 P4b build |
