...
| `CloseDocumentBubble` | repo-local `HandleDocumentBubble` delegating to `UsePairflow` `CloseBubble` | close/merge result, including `merge_conflict_recovered` when CloseBubble safely resolved and retried a bounded merge conflict; `task_source_resolution` proving artifact file source, verified route-context inline document payload, or explicit human override; `status=implementable` metadata applied in the bubble worktree before explicit conventional lifecycle commit and verified on refreshed `main`; and bubble artifact deletion or explicit retained-bubble reason |
